...
|---|---|---|---|---|---|---|
| RISK-001 | CRITICO | Schema | Nenhuma migration Prisma versionada criada — `prisma migrate deploy` no CI nao aplicara mudancas de schema | Aberto | dev-fullstack-1 | Antes do deploy em staging |
| RISK-002 | CRITICO | CI/CD | `ci.yml` dispara em `main` e `develop` mas o repositorio usa `master` — CI nao roda em PRs para `master` | Aberto | devops-engineer | Imediato |
...
As acoes abaixo bloqueiam o deploy do Sprint 2. Nao fazer `prisma migrate deploy` em staging ou producao sem completar:

- [ ] **RISK-001**: `npx prisma migrate dev --name sprint-2-schema-changes` — gerar, revisar e commitar migration SQL
- [ ] **RISK-002**: Atualizar `ci.yml` — substituir `main` por `master` nos triggers
...
- [ ] Atualizar `"version"` em `package.json` de `"0.1.0"` para `"0.2.0"`
