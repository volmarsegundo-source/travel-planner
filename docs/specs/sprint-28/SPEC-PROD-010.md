---
spec_id: SPEC-PROD-010
title: "Gamification Score in Header"
type: product
status: Draft
version: "1.0.0"
...
created: "2026-03-11"
updated: "2026-03-11"
related_specs: [SPEC-PROD-006, SPEC-PROD-012]
---

# SPEC-PROD-010: Gamification Score in Header
...
Manual testing across v0.19.0 and v0.20.0 confirmed that testers expected to see their points, level, or progress in the header/navbar area while navigating through expedition phases. The absence of this feedback loop weakens the gamification engagement cycle: users earn points but receive no real-time reinforcement until they navigate away from the expedition flow.

This spec supersedes SPEC-PROD-006 (Sprint 27 draft) which was deferred due to the recurring bug blitz prioritization. The requirements are unchanged but the spec is now formally assigned to Sprint 28 with updated cross-references to SPEC-PROD-012 (Navigation Restructure), which introduces the "Meu Atlas" page that the header indicator links to.

**Evidence**: Manual testing feedback from v0.19.0 (NEW-001). Repeated in v0.20.0 validation. Industry benchmark: Duolingo, TripAdvisor, and Foursquare all display persistent progress indicators in the header.
...
- [ ] AC-002: The gamification indicator MUST be visible on ALL authenticated pages, including: dashboard, expedition phases 1-6, account/profile, Meu Atlas page (SPEC-PROD-012), and any other authenticated route.
...
- [ ] AC-007: The gamification indicator MUST be interactive. Clicking or tapping it MUST navigate the user to the "Meu Atlas" page (SPEC-PROD-012). If SPEC-PROD-012 is not yet implemented, clicking MUST navigate to the existing profile/gamification section as a fallback.
...
- [ ] AC-009: Points and level displayed in the header MUST be consistent with the values shown on the profile/dashboard page and the Meu Atlas page (SPEC-PROD-012). There MUST be no discrepancy caused by caching or stale state.
...
- Click-through to Meu Atlas page (SPEC-PROD-012) or fallback to profile
...
- Redesigning the full gamification profile/dashboard (covered by SPEC-PROD-012)
...
- Animation library or motion framework introduction
...
- **Engagement lift**: >= 15% increase in users visiting the full gamification profile page (driven by header click-through). Measured by analytics event comparison post-beta.
...
- SPEC-PROD-012 (Navigation Restructure): defines the "Meu Atlas" page that AC-007 links to. If SPEC-PROD-012 is implemented in the same sprint, the click target is the new Meu Atlas route. If not, fallback to profile.
...
|---------|------|--------|-------------|
| 1.0.0 | 2026-03-11 | product-owner | Initial draft for Sprint 28. Supersedes SPEC-PROD-006 (Sprint 27 draft, deferred). Updated cross-references to SPEC-PROD-012 |
