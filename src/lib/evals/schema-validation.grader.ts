 ...
 * schemas used in ai.service.ts. Catches structural drift between what
 * the prompt asks for and what the model actually returns.
 *
...
// ─── Itinerary Schemas ──────────────────────────────────────────────────────────
// Mirrors: src/server/services/ai.service.ts DayActivitySchema / DayPlanSchema
...
const DayPlanSchema = z.object({
  ...
  activities: z
    .array(DayActivitySchema)
    .min(1, "Each day needs at least one activity"),
});
...
// ─── Guide Schemas ──────────────────────────────────────────────────────────────
// Mirrors: src/types/ai.types.ts GuideSectionKey / GuideSectionData
...
const DestinationGuideContentSchema = z.object(
  Object.fromEntries(
    ...
    (typeof GUIDE_SECTION_KEYS)[number],
    typeof GuideSectionDataSchema
  >
);
...
const MAX_COUNT_DEPTH = 5;

/** Recursively count fields in an object for reporting */
function countFields(obj: unknown, depth = 0): number {
  if (depth > MAX_COUNT_DEPTH || obj === null || typeof obj !== "object") {
    return 0;
  }

  if (Array.isArray(obj)) {
    return obj.reduce(
      (sum: number, item: unknown) => sum + countFields(item, depth + 1),
      0
    );
  }
  ...
  return Object.keys(obj).reduce(
    (sum, key) =>
      sum + 1 + countFields((obj as Record<string, unknown>)[key], depth + 1),
    0
  );
}
...
export function gradeSchemaValidation(
  ...
  if (!schema) {
    return {
      pass: false,
      score: 0,
      errors: [`Unknown schema type: ${String(schemaType)}`],
      details: { schemaType },
    };
  }
  ...
  if (result.success) {
    return {
      ...
      details: {
        schemaType,
        fieldsValidated: countFields(result.data),
      },
    };
  }
...
export function gradeItinerarySemantic(output: unknown): GraderResult {
  ...
  const score =
    errors.length === 0
      ? 1.0
      : Math.max(0, 1 - errors.length * SEMANTIC_PENALTY);
...
const GUIDE_SEMANTIC_PENALTY = 0.1;
...
export function gradeGuideSemantic(output: unknown): GraderResult {
  ...
  const guide = output as Record<
    string,
    z.infer<typeof GuideSectionDataSchema>
  >;
  const errors: string[] = [];
  ...
  for (const key of GUIDE_SECTION_KEYS) {
    ...
    if (
      ...
      section.value.length < CONTENT_VALUE_MIN_LENGTH
    ) {
      errors.push(
  ...
  const score =
    errors.length === 0
      ? 1.0
      : Math.max(0, 1 - errors.length * GUIDE_SEMANTIC_PENALTY);
